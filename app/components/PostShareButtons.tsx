"use client";

import {
  BlueskyIcon,
  BlueskyShareButton,
  FacebookIcon,
  FacebookShareButton,
  ThreadsIcon,
  ThreadsShareButton,
  TwitterIcon,
  TwitterShareButton,
  WhatsappIcon,
  WhatsappShareButton,
} from "react-share";

type PostShareButtonsProps = {
  className?: string;
  title: string;
  url: string;
};

function MeneameIcon() {
  return (
    <span
      aria-hidden="true"
      className="inline-flex h-[34px] w-[34px] translate-y-[2px] items-center justify-center rounded-[10px] bg-[#ff7d12]"
    >
      <svg
        focusable="false"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 32 32"
        className="h-[34px] w-[34px]"
      >
        <g fill="#FFF">
          <path d="M27.03 7.804c-.012-.44-.617-1.916-.893-1.916l-.067.01-.03.04c-.043.055-.07.157.083.714.23 1.14.413 2.764-.68 3.712-.658.667-1.562 1.052-2.437 1.425-1.208.51-2.456 1.042-3.09 2.334-.76 2.156.583 5.37 1.563 7.716.203.496.4.963.56 1.38.044.09.05.184.01.28-.224.58-1.725 1.16-2.82 1.584-.977.378-1.316.52-1.32.704 0 .037.01.11.112.155l.02.01h.023c2.303-.027 3.968-.564 4.688-1.514.372-.49.48-1.08.312-1.708-.144-.536-.448-1.233-.8-2.04-.7-1.61-1.575-3.616-1.575-5.315.045-1.5 1.388-2.156 2.687-2.787.313-.15.61-.295.874-.445 1.64-.734 3.208-2.413 2.78-4.344z" />
          <path d="M15.32 25.348c-.423-.03-.82-.057-1.108-.1q-.481-.051-.974-.1c-1.927-.19-3.92-.387-5.528-1.5-1.79-1.34-1.916-3.96-1.124-5.748.644-1.635 1.616-3.337 2.89-5.06l.066-.09q.103.317.247.622c.812 1.687 1.77 2.585 2.92 2.745q.466.07.925.07c1.04 0 1.977-.267 2.716-.77l.38-.26-.452.08q-.254.043-.51.094c-.535.106-1.086.21-1.617.21-.617 0-1.12-.15-1.54-.46-1.882-1.203-2.623-3.85-1.582-5.67.743-1.402 2.184-2.104 4.536-2.21 2.03-.092 3.998.83 5.16 1.37.292.136.545.253.74.326.482.152 1.04.314 1.577.314.62 0 1.083-.226 1.374-.66.27-.32.3-1.006.176-1.484-.046-.178-.135-.402-.295-.462l-.15-.056.016.16c.048.444-.11.938-.397 1.256a.96.96 0 0 1-.714.34q-.135 0-.29-.043c-.53-.124-1.044-.35-1.54-.57-.18-.08-.363-.16-.54-.232-1.76-.91-3.38-1.352-4.944-1.352-.61 0-1.22.07-1.805.207-1.19.22-2.786.9-3.693 2.137-.9 1.074-1.16 2.65-.738 4.122l-.16.132-.467.38c-.704.563-1.367 1.096-2.01 2.02-1.097 1.726-2.3 4.013-1.868 6.39.404 2.53 2.847 4.146 5.05 4.434 1.068.125 2.18.185 3.4.185h.002c1.088 0 2.25-.045 3.66-.147l.037-.002.026-.024c.09-.09.08-.17.065-.212-.082-.224-.645-.286-1.91-.375z" />
          <path d="M18.398 21.133c-.044-.124-.178-.502-2.283-.502-1.357 0-3.012.17-3.363.3l-.427.16.454.034c.313.025.68.037 1.063.05 1.257.04 2.82.088 3.634.644.18.316.053 1.464-.05 2.39-.11 1.004-.17 1.582-.044 1.77l.02.03.034.01a.3.3 0 0 0 .063.008q.143 0 .267-.177c.515-.734.933-3.86.627-4.713z" />
        </g>
      </svg>
    </span>
  );
}

const shareButtons = [
  {
    Button: TwitterShareButton,
    Icon: TwitterIcon,
    label: "Twitter",
  },
  {
    Button: WhatsappShareButton,
    Icon: WhatsappIcon,
    label: "WhatsApp",
  },
  {
    Button: FacebookShareButton,
    Icon: FacebookIcon,
    label: "Facebook",
  },
  {
    Button: ThreadsShareButton,
    Icon: ThreadsIcon,
    label: "Threads",
  },
  {
    Button: BlueskyShareButton,
    Icon: BlueskyIcon,
    label: "Bluesky",
  },
] as const;

export default function PostShareButtons({
  className = "",
  title,
  url,
}: PostShareButtonsProps) {
  const meneameShareUrl = `https://www.meneame.net/submit.php?url=${encodeURIComponent(
    url,
  )}&title=${encodeURIComponent(title)}`;

  return (
    <div className={`flex flex-wrap gap-3 ${className}`}>
      {shareButtons.map(({ Button, Icon, label }) => (
        <Button
          key={label}
          url={url}
          title={title}
          aria-label={`Compartir en ${label}`}
          className="cursor-pointer"
        >
          <span className="editorial-cta editorial-share-button">
            <Icon size={34} round={false} borderRadius={12} />
          </span>
        </Button>
      ))}

      <a
        href={meneameShareUrl}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Compartir en Menéame"
        className="cursor-pointer"
      >
        <span className="editorial-cta editorial-share-button">
          <MeneameIcon />
        </span>
      </a>
    </div>
  );
}
